...
# 与asyncio的EVENT LOOP基本一致,采取epoll异步通讯方式
# 说明：select\poll\epoll区别
# 晋级：select->poll->epoll
# 仅支持水平触发+fd数量最大1024 -> 仅支持水平触发 -> 水平触发 + 边缘触发
# 全平台 -> 全平台 -> 仅Linux2.6
# ** 水平触发:将就绪的fd告诉进程，如果进程没有进行IO操作，那么下次epoll时再次报告
# ** 边缘触发:只告诉进程一次哪些fd已经处于就绪状态
